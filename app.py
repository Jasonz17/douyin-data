...
@app.route('/get_single_video_url', methods=['GET'])
def get_single_video_url():
    """
    处理获取单个抖音视频下载链接的请求。
    此函数保持无头模式，不使用持久化用户数据。
    参数：url (单个视频链接)
    """
    ...
    try:
        ...
        print("单个视频解析：已配置浏览器参数：--no-sandbox, --disable-dev-shm-usage, --disable-gpu")
        print("单个视频解析：正在创建浏览器实例 (无头)...")
        browser = ChromiumPage(co)
        print("单个视频解析：浏览器实例创建成功")
...
def get_user_videos():
    ...
    try:
        ...
        print("用户视频采集：已配置浏览器参数：--no-sandbox, --disable-dev-shm-usage, --disable-gpu")
        print(f"用户视频采集：正在创建浏览器实例 (无头，使用持久化数据目录: {user_data_dir})...")
        browser = ChromiumPage(drission_co_persistent) # 使用全局的持久化 co 实例
        print("用户视频采集：浏览器实例创建成功")
                    ...
                    try:
                        ...
                        if 'aweme/v1/web/aweme/post/' in resp_item.url:
                            ...
                            for index in video_data:
                                ...
                                print(f"提取并打印视频 (数据包 {processed_packets_count}): {extracted_video['video_id']} - {extracted_video['video_title']}")
